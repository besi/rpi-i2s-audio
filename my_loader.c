...
static struct asoc_simple_card_info snd_rpi_simple_card_info = {
        .card = "raspi_sound", // -> snd_soc_card.name
...
int hello_init(void) {
  ...
  pr_alert("Hello from RaspiSound...\n");
  return 0;
}

void hello_exit(void) {// you'll have to sudo modprobe -r the card & codec drivers manually (first?)
  platform_device_unregister(&snd_rpi_simple_card_device);
  pr_alert("Goodbye from RaspiSound...\n");
}
